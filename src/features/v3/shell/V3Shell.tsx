...
export const CREATOR_WELCOME_STORAGE_KEY_V55 = "dicetree:v55:creator-welcome-seen";
...
export function V3Shell() {
  ...
  const [creatorWelcome, setCreatorWelcome] = useState(() => {
    try { return window.localStorage.getItem(CREATOR_WELCOME_STORAGE_KEY_V55) !== "1"; }
    catch { return true; }
  });
  ...
  const [updateUnread, setUpdateUnread] = useState(() => hasUnreadUpdateV55(OFFICIAL_PATCH_HISTORY_V47[0].version));
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") {
        ...
        try { window.localStorage.setItem(CREATOR_WELCOME_STORAGE_KEY_V55, "1"); } catch { /* Optional preference. */ }
  ...
  const setRemainingResource = (kind: "gold" | "stone", rawValue: number) => {
    const value = Math.max(0, Math.round(Number.isFinite(rawValue) ? rawValue : 0));
    dispatch({ type: "setInventory", inventory: { [kind]: value + spent[kind] } });
  };

  const adjustRemainingResource = (kind: "gold" | "stone", delta: number) => {
    setRemainingResource(kind, resources.remaining[kind] + delta);
  };
  ...
  return <div className={`v3-app v41-mode-${tab}`} data-testid="v3-app">
    ...
    <header className="v3-header">
      ...
      <nav className="v3-nav v53-desktop-nav" aria-label={locale === "ko" ? "주요 화면" : "Primary views"}>
        ...
        <div className="v53-tools-menu">
          ...
          {moreOpen && <div role="menu">{TOOL_TABS.map((item) => <button role="menuitem" key={item} type="button" className={`${tab === item ? "is-active" : ""} ${item === "updates" && updateUnread ? "has-update" : ""}`} onClick={() => openTab(item)}>{tabLabel(item, locale)}{item === "updates" && updateUnread && <i aria-hidden="true" />}</button>)}</div>}
        </div>
      </nav>
      <div className="v3-header-actions">
        <button className="v53-desktop-credit" type="button" onClick={openAbout}>{locale === "ko" ? "제작자 모님" : "Created by Monim"}</button>
    ...
    {headerMenuOpen && <div className="v53-header-menu" role="menu">
      ...
      <button role="menuitem" type="button" onClick={() => { setHeaderMenuOpen(false); setDiagnosticsOpen(true); }}>{locale === "ko" ? "사이트 성능 진단" : "Site performance diagnostics"}</button>
      <button role="menuitem" type="button" onClick={() => { setHeaderMenuOpen(false); openAbout(); }}>{locale === "ko" ? "제작자 및 사이트 정보" : "Creator and site info"}</button>
    ...
    <section className="v3-resource-rail" aria-label={locale === "ko" ? "다이스 트리 재화" : "Dice Tree resources"}>
      <div className="v3-resource-item gold"><span className="v3-resource-icon">●</span><label>{locale === "ko" ? "남은 골드" : "Remaining Gold"}<input aria-label={locale === "ko" ? "남은 골드" : "Remaining Gold"} type="number" min="0" step="1000" inputMode="numeric" value={Math.max(0, resources.remaining.gold)} onFocus={(event) => event.currentTarget.select()} onChange={(event) => setRemainingResource("gold", Number(event.target.value))} /></label><small>{locale === "ko" ? "계획에서 사용" : "Plan spent"} {spent.gold.toLocaleString()}</small></div>
      <div className="v3-resource-item stone"><span className="v3-resource-icon">◆</span><label>{locale === "ko" ? "남은 다이스 코어" : "Remaining Dice Core"}<input aria-label={locale === "ko" ? "남은 다이스 코어" : "Remaining Dice Core"} type="number" min="0" step="1" inputMode="numeric" value={Math.max(0, resources.remaining.stone)} onFocus={(event) => event.currentTarget.select()} onChange={(event) => setRemainingResource("stone", Number(event.target.value))} /></label><small>{locale === "ko" ? "계획에서 사용" : "Plan spent"} {spent.stone.toLocaleString()}</small></div>
    ...
    {resourceEditorOpen && <div className="v53-sheet-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setResourceEditorOpen(false); }}><section className="v53-action-sheet v53-resource-editor v55-resource-editor" role="dialog" aria-modal="true" aria-label={locale === "ko" ? "재화 편집" : "Edit resources"}><div className="v53-sheet-handle" /><header><div><small>{locale === "ko" ? "RESOURCE CONTROL" : "RESOURCE CONTROL"}</small><h2>{locale === "ko" ? "계획 후 남을 재화를 입력하세요" : "Enter resources left after this plan"}</h2></div><button type="button" aria-label={locale === "ko" ? "재화 편집 닫기" : "Close resource editor"} onClick={() => setResourceEditorOpen(false)}>×</button></header><div className="v55-resource-grid"><article className="is-gold"><header><span aria-hidden="true">●</span><div><strong>{locale === "ko" ? "골드" : "Gold"}</strong><small>{locale === "ko" ? `계획 전 총 ${state.inventory.gold.toLocaleString()}` : `Total before plan ${state.inventory.gold.toLocaleString()}`}</small></div></header><label>{locale === "ko" ? "계획 후 남은 골드" : "Gold remaining after plan"}<input autoFocus aria-label={locale === "ko" ? "계획 후 남은 골드" : "Gold remaining after plan"} type="number" min="0" step="1000" inputMode="numeric" value={Math.max(0, resources.remaining.gold)} onFocus={(event) => event.currentTarget.select()} onChange={(event) => setRemainingResource("gold", Number(event.target.value))} /></label><output>{Math.max(0, resources.remaining.gold).toLocaleString()} G</output><div className="v55-resource-presets" aria-label={locale === "ko" ? "골드 빠른 추가" : "Quick Gold add"}>{[10000, 50000, 100000].map((amount) => <button key={amount} type="button" onClick={() => adjustRemainingResource("gold", amount)}>+{amount.toLocaleString()}</button>)}<button type="button" onClick={() => setRemainingResource("gold", 0)}>0</button></div></article><article className="is-core"><header><span aria-hidden="true">◆</span><div><strong>{locale === "ko" ? "다이스 코어" : "Dice Core"}</strong><small>{locale === "ko" ? `계획 전 총 ${state.inventory.stone.toLocaleString()}` : `Total before plan ${state.inventory.stone.toLocaleString()}`}</small></div></header><label>{locale === "ko" ? "계획 후 남은 다이스 코어" : "Dice Core remaining after plan"}<input aria-label={locale === "ko" ? "계획 후 남은 다이스 코어" : "Dice Core remaining after plan"} type="number" min="0" step="1" inputMode="numeric" value={Math.max(0, resources.remaining.stone)} onFocus={(event) => event.currentTarget.select()} onChange={(event) => setRemainingResource("stone", Number(event.target.value))} /></label><output>{Math.max(0, resources.remaining.stone).toLocaleString()} C</output><div className="v55-resource-presets" aria-label={locale === "ko" ? "다이스 코어 빠른 추가" : "Quick Dice Core add"}>{[10, 50, 100].map((amount) => <button key={amount} type="button" onClick={() => adjustRemainingResource("stone", amount)}>+{amount.toLocaleString()}</button>)}<button type="button" onClick={() => setRemainingResource("stone", 0)}>0</button></div></article></div><div className={`v55-resource-summary ${resources.affordable ? "is-ok" : "is-short"}`}><strong>{resources.affordable ? (locale === "ko" ? "현재 계획을 감당할 수 있습니다" : "This plan is affordable") : (locale === "ko" ? "현재 계획에 재화가 부족합니다" : "This plan exceeds your resources")}</strong><span>{locale === "ko" ? `가상 계획에서 ${spent.gold.toLocaleString()} 골드와 ${spent.stone.toLocaleString()} 코어가 자동 차감됩니다.` : `${spent.gold.toLocaleString()} Gold and ${spent.stone.toLocaleString()} Core are deducted automatically.`}</span></div><button className="is-primary" type="button" onClick={() => setResourceEditorOpen(false)}>{locale === "ko" ? "적용하고 닫기" : "Apply and close"}</button></section></div>}
    ...
    {aboutOpen && <div className={`v53-sheet-backdrop v55-creator-backdrop ${creatorWelcome ? "is-welcome" : ""}`} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeAbout(); }}><section className="v53-action-sheet v54-about-sheet v55-creator-sheet" role="dialog" aria-modal="true" aria-labelledby="v54-about-title"><div className="v53-sheet-handle" /><header><div><small>{creatorWelcome ? "WELCOME TO DICETREE" : "DICETREE · COMMUNITY TOOL"}</small><h2 id="v54-about-title">{locale === "ko" ? "제작자 모님" : "Created by Monim"}</h2></div><button autoFocus type="button" aria-label={locale === "ko" ? "사이트 정보 닫기" : "Close site info"} onClick={closeAbout}>×</button></header><div className="v54-about-mark" aria-hidden="true"><b>RD</b><i>2</i></div>{creatorWelcome && <p className="v55-welcome-line">{locale === "ko" ? "다이스 트리 투자를 결정하기 전에, 비용과 성능을 먼저 검증하세요." : "Verify cost and performance before committing a Dice Tree investment."}</p>}<p className="v54-about-intro">{locale === "ko" ? "게임 안에서 재화를 쓰기 전에 다이스 트리 투자, 덱 구성, 전투 결과를 안전하게 검토할 수 있도록 만든 랜덤 다이스 2 비공식 팬 도구입니다." : "An unofficial Random Dice 2 fan tool for reviewing Dice Tree investments, deck composition, and combat results before spending resources in game."}</p><div className="v54-about-principles"><article><strong>{locale === "ko" ? "근거를 구분합니다" : "Evidence stays visible"}</strong><span>{locale === "ko" ? "검증된 계산, 부분 검증, 추정을 결과마다 표시합니다." : "Every result distinguishes verified, partial, and estimated calculations."}</span></article><article><strong>{locale === "ko" ? "사용자 선택을 우선합니다" : "Your choices come first"}</strong><span>{locale === "ko" ? "추천 이유와 비용을 보여주고 사용자가 경로를 다시 조정할 수 있게 합니다." : "Recommendations expose their reasons and costs so routes remain editable."}</span></article></div><a className="v54-about-link" href="https://github.com/woo642778-art/dicetree" target="_blank" rel="noreferrer">{locale === "ko" ? "GitHub에서 DiceTree 보기" : "View DiceTree on GitHub"}</a><small className="v54-about-disclaimer">{locale === "ko" ? "DiceTree는 게임 제작사와 제휴되지 않은 비공식 팬 프로젝트입니다." : "DiceTree is an unofficial fan project and is not affiliated with the game's publisher."}</small></section></div>}
